...
public class LatestArrivalPathProcedure {
    ...
    @Procedure(value="consecutive.latestArrival")
    @Description("Get the path with the latest interval.")
    public Stream<TemporalPathRecord> latestArrival(
        ...
        PathStrategy<Long> strategy = new ComparatorPruneConsecutiveStrategy(
                log,
                Comparator.comparing(TimeNode::getTime),
                (solution, other) -> solution.getTime().compareTo(other.getTime()) > 0,
                difference,
                (n1, n2) -> {}
        );
