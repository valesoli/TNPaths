...
public class LatestDeparturePathProcedure {
    ...
    @Procedure(value="consecutive.latestDeparture")
    @Description("Get the path with the latest departure interval.")
    public Stream<TemporalPathRecord> latestDeparture(
        ...
        PathStrategy<Long> strategy = new ComparatorPruneConsecutiveStrategy(
                ...
                (n1, n2) -> {
                    ...
                    if (n2.getPayload() == null)
                        return 1;
                    return n1.getPayload().compareTo(n2.getPayload());
                },
                (solution, other) -> solution.getPayload() > other.getPayload(),
                difference,
        ...
        timer.stop();
        log.info(String.format("Algorithm finished in %sms", (timer.elapsed().toNanos() / (double) 1000000)));
