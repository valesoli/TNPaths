...
public class OriginalCreateTreeIndex {
    ...
    @Procedure(value="graphindex.createOriginalIndex", mode=Mode.WRITE)
    ...
    public void createTreeIndex(
        ...
        StringBuilder newIndexNodeBuilder = new StringBuilder()
            ...
            .append("WHERE n.MaxLength = $maxLength and n.MinLength = $minLength and n.From = $startTime and n.To = $endTime \n")
            .append("RETURN n");
        ...
        if (!newIndexNodeResult.hasNext()) {
            StringBuilder createNewIndexNodeStatement = new StringBuilder()
                .append("CREATE (n:INDEX:META:").append(relationship).append("{MaxLength: $maxLength, MinLength: $minLength, From: $startTime, To: $endTime})\n")
        ...
        objectIdsResult.stream().forEach(objectId -> {
            ...
            // Result otherIdsResult = db.execute(idsAtLQueryBuilder.toString(), cPathsParams);
                ...
                StringBuilder cPathsQueryBuilder = new StringBuilder()
                    ...
                    .append("WHERE u1.id = $objectId\n")
                    .append("CALL coexisting.coTemporalPaths(u1,null,").append(minLength).append(",").append(maxLength)
                    .append(",{edgesLabel: $relationship, between: '").append(startTime).append("—").append(endTime).append("',").append("direction:'outgoing'})\n")
                ...
                if (!cPathsResults.hasNext()){
                    ...
                    cPathsResults.stream().forEach(pathsRecord -> {
                        ...
                        for (int i = 1; i < path.size() -1; i++) intermediate.add((Long) path.get(i).get("id"));
                        long destinationId = (Long) path.get(path.size()-1).get("id");
                        ...
                        for (String interval : intervals) {
                            ...
                            createdIndexNodeParams.put("pathLength", path.size()-1);
